...
$(document).ready(function() {
  $('.more.row').on('click', function(e) {
    e.preventDefault();
  })
  $("#launches .tile.pillar nav").nanoScroller();
  $('.pillar-toggle').on('click', function(e) {
    e.preventDefault();
    $(this).toggleClass('open');
    $('.pillar-menu').toggleClass('open');
  });
});

function togglePersistant() {
  if($(window).scrollTop() < -($(window).height() - $('#ferrariFooter').offset().top)) {
    $('.more.row').removeClass('top')
  } else {
    $('.more.row').addClass('top')
  }
}

$(window).on('scroll', togglePersistant);
$(document).on('ready', togglePersistant);

$('.more-link').on('click', function() {
  if (!$(this).closest('.more').hasClass('top')) {
    var pos = $('.wndw').eq(1).offset().top - $('.fe-nav-bar').outerHeight();
